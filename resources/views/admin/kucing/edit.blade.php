...
@section('content_header')
    <h1>Edit Kucing: {{ $kucing->nama_kucing }}</h1>
@stop
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">
            <i class="fas fa-edit"></i> Form Edit Kucing
        </h3>
    </div>
    <div class="card-body">
        @if($errors->any())
        ...
        <form action="{{ route('admin.users.kucing.update', [$user->id, $kucing->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            
            {{-- Upload/Edit Foto Kucing --}}
            <div class="form-group">
                <label for="gambar">Foto Kucing</label>
                
                {{-- Current Image atau Upload Area --}}
                <div class="upload-area" id="upload-area" style="{{ $kucing->gambar ? 'display: none;' : '' }}">
                ...
                {{-- Preview Area --}}
                <div class="image-preview-container" id="preview-container" style="{{ $kucing->gambar ? '' : 'display: none;' }}">
                    <img id="image-preview" 
                         src="{{ $kucing->gambar ? asset('storage/' . $kucing->gambar) : '#' }}" 
                    ...
                    @if($kucing->gambar)
                        <small class="text-muted d-block mt-2">Foto saat ini akan diganti jika Anda upload foto baru</small>
                    @endif
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('nama_kucing', $kucing->nama_kucing) }}" 
                               required>
                        @error('nama_kucing')
            ...
            <div class="form-group">
                ...
                <textarea id="riwayat_kesehatan" 
                          ...
                          rows="4"
                          placeholder="Masukkan riwayat kesehatan kucing (vaksin, penyakit, operasi, dll)">{{ old('riwayat_kesehatan', $kucing->riwayat_kesehatan) }}</textarea>
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Update Kucing
                </button>
        ...
        <div class="mt-4 pt-3" style="border-top: 1px solid #dee2e6;">
            ...
            <p class="text-muted">Tindakan ini tidak dapat dibatalkan. Data kucing akan dihapus permanen.</p>
            <form action="{{ route('admin.users.kucing.destroy', [$user->id, $kucing->id]) }}" method="POST" style="display: inline;">
                @csrf
                @method('DELETE')
                <button type="button" class="btn btn-danger" onclick="confirmDelete()">
                    <i class="fas fa-trash"></i> Hapus Data Kucing
                </button>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnRemovePhoto.addEventListener('click', function() {
        ...
        if (confirmDelete) {
            ...
            // Jika ada foto original, tampilkan kembali
            if (originalImageSrc && originalImageSrc.includes('storage/')) {
                imagePreview.src = originalImageSrc;
            } else {
                // Jika tidak ada foto original, kembali ke upload area
...
function confirmDelete() {
    const confirmDelete = confirm('Yakin ingin menghapus data kucing {{ $kucing->nama_kucing }}? Tindakan ini tidak dapat dibatalkan!');
    if (confirmDelete) {
        // Submit form delete
